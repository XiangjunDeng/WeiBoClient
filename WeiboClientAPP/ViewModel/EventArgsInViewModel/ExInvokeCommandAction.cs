...
{
    public class ExInvokeCommandAction : TargetedTriggerAction<DependencyObject>
    {
        ...
        private ICommand ResolveCommand()
        {
            ...
            else
            {
                if (base.AssociatedObject != null)
                {
                    ...
                    IEnumerable<PropertyInfo> array = properties;
                    for (int i = 0; i < array.Count(); i++)
                    {
                        PropertyInfo propertyInfo = array.ElementAt(i);
                        if (typeof(ICommand).GetElementType() == (propertyInfo.PropertyType) && string.Equals(propertyInfo.Name, this.CommandName, StringComparison.Ordinal))
                        {
